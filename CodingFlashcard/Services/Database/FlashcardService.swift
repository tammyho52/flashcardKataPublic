...
final class FlashcardService {
    ...
    init() {
        self.databaseService = FirestoreService<Flashcard, Date>(collectionPath: collectionPath, orderByKeyPath: \.updatedDate, orderDirection: .descending)
    }
    ...
    /// Fetch and search on client side due to Firestore not allowing for "contains substring" searches and case-insensitive searches
    func fetchSearchFlashcards(for searchText: String, userID: String) async throws -> [Flashcard] {
        ...
        let filteredFlashcards = flashcards.filter {
            $0.frontText.lowercased().contains(caseInsensitiveSearchText) || $0.backText.lowercased().contains(caseInsensitiveSearchText)
        }
    ...
    func query(firstPredicates: [QueryPredicate], secondPredicates: [QueryPredicate], userID: String) async throws -> [Flashcard] {
    ...
    func queryPaginatedFlashcards(predicates: [QueryPredicate], userID: String, lastDocument: DocumentSnapshot?) async throws -> [Flashcard] {
