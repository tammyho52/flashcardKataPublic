...
final class ReviewViewModel: ObservableObject {
    ...
    private func updateReviewSessionSummary() {
        let numberOfDecks = flashcardDisplayModels.reduce(into: [:]) { result, flashcardModel in result[flashcardModel.deckNameLabel.id, default: 0] += 1 }
    ...
    func saveReviewSessionSummary() async throws {
        ...
        for flashcardID in reviewSessionSummary.flashcardReviewResults.keys {
            try await databaseManager.updateFlashcard(updates: [FlashcardUpdate.recentReviewedDate(reviewSessionSummary.completedDate)], for: flashcardID)
        }
        
        let reviewedDisplayModels = flashcardDisplayModels.filter { reviewSessionSummary.flashcardReviewResults.keys.contains($0.flashcard.id) }
        ...
        for deckID in reviewedDecks {
            try await databaseManager.updateDeck(updates: [DeckUpdate.lastReviewedDate(reviewSessionSummary.completedDate)], for: deckID)
        }
    ...
    var completedReviewModeMessage: String {
        ...
        case .timed:
            if isTimerEnded, let sessionTime = reviewSettings.sessionTime {
                ...
                return "Completed Kata Time: \(TimeConverter(totalSeconds: getReviewSessionTimeInSeconds()).formattedShortTime())"
            }
        }
    }
}
