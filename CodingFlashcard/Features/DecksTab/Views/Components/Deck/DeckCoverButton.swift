...
//
// MARK: - Purpose
// View that displays a button displaying deck information. The button triggers an action when tapped, typically for navigating to a deck or viewing more details.
...
struct DeckCoverButton: View {
    ...
    var body: some View {
        Button(action: navigateAction) {
            ...
            .applyDeckCoverBackgroundGradient(primaryColor: deck.theme.primaryColor, secondaryColor: deck.theme.secondaryColor, isSubdeck: deck.isSubdeck)
            .clipDefaultShape()
            .applyCoverShadow()
        }
    ...
    private var formattedFlashcardCount: String {
        pluralize(word: .flashcard, count: deck.flashcardCount).capitalized
    }
    
    private var formattedSubdeckCount: String {
        pluralize(word: .subdeck, count: deck.subdeckIDs.count).capitalized
    }
}
