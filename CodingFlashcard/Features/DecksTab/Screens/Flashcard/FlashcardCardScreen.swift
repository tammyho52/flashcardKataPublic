    ...
    var body: some View {
            ...
            if isLoading {
                ...
                VStack {
                    ...
                    FlashcardCardView(
                        ...
                        isFlippable: true,
                        flashcard: flashcard,
                        deckNameLabel: DeckNameLabel(id: deck.id, parentDeckID: deck.parentDeckID, name: deck.name, theme: deck.theme, isSubDeck: deck.isSubdeck)
                    )
                    .padding(20)
                    Spacer()
                }
        ...
        .onAppear {
            Task {
                ...
                if let selectedDeckID, let deck = try await vm.fetchDeck(id: selectedDeckID) {
                    selectedDeck = deck
                }
                if let selectedFlashcardID, let flashcard = try await vm.fetchFlashcard(id: selectedFlashcardID) {
...
#Preview {
    NavigationStack {
        FlashcardCardScreen(
            vm: FlashcardFormViewModel(databaseManager: MockDatabaseManager()),
