...
struct FlashcardCardView: View {  
    ...
    private struct FlashcardBackground: ViewModifier {
        ...
        func body(content: Content) -> some View {
            content
                .padding(5)
                .background(LinearGradient(colors: [.white, Color.customBackground.opacity(0.5)], startPoint: .topLeading, endPoint: .bottomTrailing))
...
extension FlashcardCardView {
    init(showFlashcardFront: Binding<Bool>, isFlippable: Bool, flashcard: Flashcard, reviewSettings: ReviewSettings = ReviewSettings(showHint: true, showDifficultyLevel: true, showFlashcardDeckName: true), deckNameLabel: DeckNameLabel, aspectRatio: Double = 0.65) {
...
#if DEBUG
#Preview {
    FlashcardCardView(showFlashcardFront: .constant(true), isFlippable: true, flashcard: Flashcard.sampleFlashcard, deckNameLabel: DeckNameLabel.sample)
}

#Preview {
    FlashcardCardView(showFlashcardFront: .constant(false), isFlippable: false, flashcard: Flashcard.sampleFlashcard, deckNameLabel: DeckNameLabel.sample)
}
#endif
