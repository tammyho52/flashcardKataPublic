...
//  Created by Tammy Ho
//
//  View model for Tracker tab, including fetching review session summaries and calculating session summary statistics.
...
final class TrackerViewModel: ObservableObject {
    ...
    private func updateCalendarPieChartData() async throws {
        var flashcardIDs: [String: Bool] = [:]
        var deckFlashcardData: [Deck: [String: Bool]] = [:]

        for reviewSessionSummary in reviewSessionSummaries {
            flashcardIDs = flashcardIDs.merging(reviewSessionSummary.flashcardReviewResults) { current, new in
        ...
        for (id, isCorrect) in flashcardIDs {
            do {
        ...
        var consolidatedChartItems: [ChartItem] = deckFlashcardData.map {
            ChartItem(deck: $0.0, flashcardsReviewed: $0.1)
        }
