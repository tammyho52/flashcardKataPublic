...
//  Created by Tammy Ho.
//
//  Structure that abstracts interactions with Firestore for CRUD operations, batch operations, and queries.
...
struct FirestoreService<
    Document: Codable & Firestorable & Equatable & Hashable,
    T: Comparable
> {
    ...
    /// Updates specific fields of a document in Firestore based on the provided operations and associated values.
    ...
    func query(predicates: [QueryPredicate], userID: String, documentLimit: Int? = nil) async throws -> [Document] {
        ...
        while !isLastBatch {
            ...
            let batchQuery = try await createQuery(
