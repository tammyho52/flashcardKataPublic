...
//  Created by Tammy Ho.
//
//  Manages the app's data flow and interactions with the backend, including
//  fetching, updating, and deleting of decks, flashcards, and review session summaries.
...
final class DatabaseManager: ObservableObject, DatabaseManagerProtocol {
    ...
    func fetchAllParentDecks(deckCountLimit: Int? = nil) async throws -> [Deck] {
        do {
            ...
            throw DatabaseManagerError.deckFetchError(error: error)
        }
    }
    ...
    func fetchInitialParentDecks() async throws -> [Deck] {
        ...
        if cacheDecks.count >= initialFetchItemLimit {
            ...
            let fetchedDecks = try await deckService.queryPaginatedDecks(
    ...
    func fetchMoreFlashcards(lastFlashcardID: String, flashcardIDs: [String]) async throws -> [Flashcard] {
        if let lastIndex = flashcardIDs.firstIndex(of: lastFlashcardID) {
            let limitedFlashcardIDs = Array(flashcardIDs[lastIndex + 1 ..< min(lastIndex + 11, flashcardIDs.count)])
    ...
    func createDeck(deck: Deck) async throws {
        do {
            ...
            throw DatabaseManagerError.deckSaveError(error: error)
        }
    }
    ...
    func deleteDeck(by id: String) async throws {
        do {
            ...
            throw DatabaseManagerError.deckDeleteError(error: error)
        }
    }
    ...
    func updateDeck(updates: [DeckUpdate], for id: String) async throws {
        do {
            ...
            throw DatabaseManagerError.deckEditError(error: error)
        }
    }
    ...
    func fetchSearchFlashcards(for searchText: String) async throws -> [SearchResult] {
        do {
            ...
            throw DatabaseManagerError.flashcardFetchError(error: error)
        }
    }
